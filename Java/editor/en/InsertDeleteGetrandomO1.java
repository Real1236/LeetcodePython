...
called. 
 ...
 Related Topics Array Hash Table Math Design Randomized 👍 6267 👎 326
...
class RandomizedSet {
    ...
    public boolean insert(int val) {
        if (this.valueIndex.containsKey(val))
            return false;

        this.valueList.add(val);
        this.valueIndex.put(val, this.valueList.size() - 1);
        return true;
    }
    ...
    public boolean remove(int val) {
        ...
        int indexRemoved = this.valueIndex.get(val);
        int lastValue = this.valueList.get(this.valueList.size() - 1);

        this.valueList.set(indexRemoved, lastValue);
        this.valueIndex.put(lastValue, indexRemoved);

        this.valueList.remove(this.valueList.size() - 1);
        this.valueIndex.remove(val);
        return true;
    }
    
    public int getRandom() {
        return this.valueList.get(ThreadLocalRandom.current().nextInt(0, this.valueList.size()));
    }
}
