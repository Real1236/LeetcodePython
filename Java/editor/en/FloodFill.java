...
the image.
 ...
 Related Topics Array Depth-First Search Breadth-First Search Matrix 👍 5670 👎 
554
...
class Solution {
    ...
    private void dfs(int[][] image, int row, int col, int startColor, int endColor) {
        if (image[row][col] != startColor) return;
        if (image[row][col] == endColor) return;
        image[row][col] = endColor;
        if (row > 0) dfs(image, row - 1, col, startColor, endColor);
        if (col > 0) dfs(image, row, col - 1, startColor, endColor);
        if (row < image.length - 1) dfs(image, row + 1, col, startColor, endColor);
        if (col < image[row].length - 1) dfs(image, row, col + 1, startColor, endColor);
    }
}
