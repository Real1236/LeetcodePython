...
node. 
 ...
 Related Topics Hash Table Depth-First Search Breadth-First Search Graph 👍 6852
 👎 2837
...
class Solution {
    ...
    private Node dfs(Node current, Map<Integer, Node> visited) {
        if (current == null) return null;

        if (visited.containsKey(current.val)) return visited.get(current.val);
