...
Explanation: We cannot reach the target without getting stuck.
 ...
 Related Topics Array Hash Table String Breadth-First Search 👍 3245 👎 115
...
class Solution {
    ...
    private List<String> getChildren(String parent) {
        ...
        for(int i = 0; i < parent.length(); i++) {
            ...
            result.add(parent.substring(0, i)
                    + (Integer.parseInt(String.valueOf(ch)) + 1) % 10
                    + parent.substring(i + 1));
            result.add(parent.substring(0, i)
                    + ((Integer.parseInt(String.valueOf(ch)) - 1) + 10) % 10
                    + parent.substring(i + 1));
        }
        return result;
    }
